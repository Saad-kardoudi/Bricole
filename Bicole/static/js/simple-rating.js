...
class simpleRating{
  ...
  init(){
    ...
    for(var i=0;i<5;i++){html+='<i class="bx bx-star" data-rating="'+(i+1)+'"></i>';}
    html+='</div>';
  ...
  setstars(rating){
    ...
    for(var i=0;i<5;i++){
      ...
      if(i<rating){
        $(starObj).addClass('bxs-star');
        $(starObj).removeClass('bx-star');
      }else{
        $(starObj).addClass('bx-star');
        $(starObj).removeClass('bxs-star');
      }
    }
  }
}